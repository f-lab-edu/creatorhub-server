# 작품 이미지 업로드 및 리사이징 처리 
작품 이미지 업로드 & 리사이징 처리에서 발생한 ISSUE와 해결과정을 정리한 문서입니다.
...
- 이미지 업로드 및 리사이징 실패시 재처리 가능해야 하며 운영자가 원인을 추적할 수 있어야 함
...
- **이유**: 파일 업로드를 프론트엔드에서 S3로 직접 처리하여 서버 부하 감소 및 비용 절약
...
- **이유:** 
  - 리사이징 이미지의 갯수는 확장 될 가능성 있으므로 서버 성능을 위해 Lambda 처리  
  - 포스터형, 가로형 이미지는 사용자 미리보기가 중요하여 프론트엔드에서 즉시 리사이징한다. 다만 최종 업로드 결과물의 규격(사이즈/MIME)은 서버에서 검증하여 일관성을 유지한다.
      반면 '리사이징된 이미지 6장'은 플랫폼이 규칙 기반으로 생성하는 파생 리소스이므로, 결과 품질과 규격을 서버 측에서 강제하기 위해 AWS Lambda로 비동기 생성한다.
...
- **이유:** 리사이징은 비동기 처리 특성상 간헐정 장애가 발생 할 수 있어 SQS 재시도로 자동 복구율을 높임. 반복 실패 메시지는 DLQ로 격리해 무한 재시도를 방지하고, Slack 알림으로 운영자가 즉시 대응할 수 있도록 함.

### **4. 네번째 ISSUE: Lambda의 비동기 동작 완료 후 프론트엔드에 결과 통보 방식**
- **고민:** 폴링(Polling) vs SSE(Server-Sent Events)
- **해결방법: SSE 이용**
- **이유:** 
  - 폴링은 요청이 반복되어 서버 및 DB 부하가 누적되지만 SSE는 업로드 직후 짧은 시간 동안만 연결을 유지하면서 완료 이벤트를 서버가 즉시 push할 수 있어, 불필요한 트래픽을 줄인다. 
  - 폴링의 interval, 재시도, 타임아웃 관리 로직을 제거해 클라이언트 구현을 단순화
